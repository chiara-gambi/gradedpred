...
print(all_comb)
ggsave("growthcurve.CI_pred_children_elog_raw.pdf", device = "pdf",plot=all_comb, width=18, height=54, unit="cm", dpi=600, path=getwd())
dev.off()
...
## C-biasing model
mdiffCBA.AV<-lmer(Elog~1+(t1+t2)*(BD1+BD2)*(AC+VC)+t1*(1+t1+t2|Participant)+(1+t1+t2|Participant:BD1)+(1+t1+t2|Participant:BD2),data=subset(gca,Contrast=="CBA"),REML=F)
summary(mdiffCBA.AV)
confint(mdiffCBA.AV, method="Wald")
## A-biasing model
mdiffABC.AV<-lmer(Elog~1+(t1+t2)*(BD1+BD2)*(AC+VC)+(1+t1+t2|Participant)+(1+t1+t2|Participant:BD1)+(1+t1+t2|Participant:BD2),data=subset(gca,Contrast=="ABC"),REML=F)
summary(mdiffABC.AV)
...
diff.I<-diff.I[,-7]
diff.I<-reshape(diff.I,varying=names(diff.I)[4:6],direction="long")
...
print(rec_cost_all)
ggsave("CI_reccost_children_prop_tff.pdf", device="pdf", plot=rec_cost_all, width=45, height=15, unit="cm", dpi=600, path=getwd())
```
...
```{r rev correlations}
predictReuter <- data.frame(Participant = spwa.subj.w$Participant, PredR= rowMeans(spwa.subj.w[,c("BFIXBIN.ABC","BFIXBIN.CBA")]))
...
all<-merge(all,tff_raw,by=c("Participant","Age","BPVS"),sort=F)
# Correlations
...
#Figure 4, main text
library(ggpubr)
pred_rec_predrev_raw_allc<-ggarrange(pred_raw_allc,predrev_raw_allc,rec_raw_allc,labels = c("A","B","C"), nrow=1)
print(pred_rec_predrev_raw_allc)
ggsave("rec_pred_predrev_raw_all_crosssect.pdf", device="pdf",plot=pred_rec_predrev_raw_allc, width=45, height=15, unit="cm", dpi=600, path=getwd())
...
data<-read.csv("https://raw.githubusercontent.com/chiara-gambi/gradedpred/master/phase2/long_part_info.csv",header=T)
...
ggsave("rec_pred_predrev_raw_all.pdf", device="pdf", plot=pred_rec_predrev_raw_all, width=45, height=15, unit="cm", dpi=600, path=getwd())
...
names(graph2.raw)[38]<-"Speed"
names(graph2.raw)[37]<-"Rev"
names(graph2.raw)[35]<-"Pred"
